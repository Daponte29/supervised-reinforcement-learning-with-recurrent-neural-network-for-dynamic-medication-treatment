CREATE TABLE mimic-iii-srl-rnn.SRL_RNN.vital_sign AS
with ce as
(
...
from ce
left join `physionet-data.mimiciii_clinical.admissions` ad on ce.HADM_ID=ad.HADM_ID
group by ce.HADM_ID, ce.icustay_id, ce.charttime
order by ce.HADM_ID,ce.icustay_id, ce.charttime)
select HADM_ID,cast(TRUNC(DATETIME_DIFF(charttime,ADMITTIME,MINUTE)/1440)as INT)  as time_stamp
  , avg(heartrate) as heartrate
  , avg(sysbp) as sysbp
  , avg(diasbp) as diasbp
  , avg(meanbp) as meanbp
